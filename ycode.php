...
foreach(array(
	...
	"/$URL_PATTERN/" => function($url)
	{
		if(substr($url, 0, 3) == "www") $url = "http://".$url;
		$url_data = parse_url($url);
		if(isset($url_data["query"]) && preg_match("/youtube\\.[a-z]/i", $url_data["host"]) && preg_match("/v=([a-z0-9_-]{11})/i", $url_data["query"], $match))
		...
		elseif(isset($url_data["query"]) && preg_match("/liveleak\\.com/i", $url_data["host"]) && preg_match("/i=([^&]+)/", $url_data["query"], $match))
		...
		elseif(isset($url_data["query"]) && preg_match("/dailymotion\\.com/i", $url_data["host"]) && preg_match("/video\\/([^\\/]+)/i", $url_data["query"], $match))
